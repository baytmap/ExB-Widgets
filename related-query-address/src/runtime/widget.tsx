...
const Widget = (props: AllWidgetProps<IMConfig>) => {
  ...
  const [geom, setGeom] = React.useState<any[]>([]);
  const [geom2, setGeom2] = React.useState<any[]>([]);
  const [geom3, setGeom3] = React.useState<any[]>([]);
  const [geom4, setGeom4] = React.useState<any[]>([]);
  ...
  const handleIlceSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // extract data
      setMahalle(features.map((f: any) => f.attributes));
  ...
  const handleMahalleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await layers.queryTaskMah.queryRelatedFeatures(
        relatedQuery2
      );
      ...
      const result2 = await layers.queryTaskYolOrtaHatYon.queryRelatedFeatures(
        relatedQuery3
      );
      ...
      const result3 = await layers.queryTaskYolOrtaHat.queryRelatedFeatures(
        relatedQuery6
      );
  ...
  const handleYolSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await layers.queryTaskYol.queryRelatedFeatures(
        relatedQuery7
      );
      ...
      const result2 = await layers.queryTaskYolOrtaHat.queryRelatedFeatures(
        relatedQuery4
      );
      ...
      const result3 = await layers.queryTaskYolOrtaHatYon.queryRelatedFeatures(
        relatedQuery5
      );
  ...
  return (
    <div className="adres-widget">
      ...
      <br />
      <form className="pure-form" onSubmit={handleIlceSubmit}>
        <select
      ...
      <br />
      <form className="pure-form2" onSubmit={handleMahalleSubmit}>
        <select
          className="adres-select"
      ...
      <br />
      <form className="pure-form3" onSubmit={handleYolSubmit}>
        <select
          className="adres-select"
          value={selectedYol ?? ""}
      ...
      <br />
      <form className="pure-form4" onSubmit={handleNumaratajSubmit}>
        <select
          className="adres-select"
